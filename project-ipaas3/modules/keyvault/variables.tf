...
variable "key_vault_name" {
  description = "Name of the Key Vault (must be globally unique, 3-24 characters, alphanumeric and hyphens only, must end with 2-digit suffix)"
  type        = string

  validation {
    condition     = can(regex("^[a-zA-Z0-9-]{3,24}$", var.key_vault_name)) && can(regex("[0-9]{2}$", var.key_vault_name))
    error_message = "Key Vault name must be 3-24 characters, alphanumeric and hyphens only, with 2-digit suffix (e.g., 01)"
  }
}
